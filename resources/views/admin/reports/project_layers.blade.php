            ...
            <div class="card border shadow-none">
                <div class="card-body">
                    <div class="d-flex justify-content-between mb-3">
                        <input type="text" id="treeSearch" class="form-control form-control-sm w-25"
                               placeholder="Search project or layer...">
                        ...
                        <button class="btn btn-primary btn-sm px-3" data-bs-toggle="modal"
                                ...
                                onclick="$('#projectForm')[0].reset(); $('#edit_p_id').val('');">+ New Project
                        </button>
                    </div>
                                ...
                                <li class="dd-item dd-nodrag" data-id="p{{ $project->id }}">
                                    <div class="dd-handle"
                                        ...
                                        <div class="t-col c-users">
                                            @if($project->user)
                                                <span class="user-tag"
                                                      style="background:#e0f2fe">{{ $project->user->name }}</span>
                                            @endif
                                        </div>
                                        <div class="t-col c-action">
                                            <button type="button"
                                                    class="btn btn-success btn-xs open-add-modal py-0 px-2"
                                                    data-project="{{ $project->id }}" data-parent="">+ Layer
                                            </button>
                                            <a href="javascript:;" class="edit-project ms-2 text-primary"
                                               data-id="{{ $project->id }}"><i class="bx bx-edit-alt"></i></a>
                                            <a href="javascript:;" class="delete-project ms-2 text-danger"
                                               data-id="{{ $project->id }}"
                                               data-has-child="{{ $project->layers->count() > 0 ? 'yes' : 'no' }}"><i
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                <form id="layerForm">@csrf
                    ...
                    <div class="modal-header bg-dark text-white py-2"><h6 class="modal-title">Layer Setup</h6>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body p-4">
                        <div class="mb-3"><label class="fw-bold small">Layer Name</label><input type="text" name="name"
                                                                                                id="modal_name"
                                                                                                class="form-control"
                                                                                                required></div>
                        <div class="mb-3"><label class="fw-bold small">Assign Users</label><select name="user_ids[]"
                                                                                                   id="layer_users"
                                                                                                   class="form-control select2-multiple"
                                                                                                   multiple="multiple">@foreach($users as $u)
                                    <option value="{{$u->id}}">{{$u->name}}</option>
                                @endforeach</select></div>
                        <div class="row">
                            <div class="col-6 mb-3"><label class="small fw-bold">Status</label><select name="status_id"
                                                                                                       id="modal_status_id"
                                                                                                       class="form-select">@foreach($statuses as $s)
                                        <option value="{{$s->id}}">{{$s->label}}</option>
                                    @endforeach</select></div>
                            <div class="col-6 mb-3"><label class="small fw-bold">Start Date</label><input type="date"
                                                                                                          name="start_time"
                                                                                                          id="modal_start_time"
                                                                                                          class="form-control">
                            </div>
                            <div class="col-12 mb-3"><label class="small fw-bold">End Date</label><input type="date"
                                                                                                         name="end_time"
                                                                                                         id="modal_end_time"
                                                                                                         class="form-control">
                            </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                <form id="projectForm">@csrf
                    ...
                    <div class="modal-header bg-primary text-white py-2"><h6 class="modal-title">New Project</h6>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body p-4">
                        <div class="mb-3"><label class="fw-bold small">Title</label><input type="text" name="title"
                                                                                           id="p_title"
                                                                                           class="form-control"
                                                                                           required></div>
                        <div class="mb-3"><label class="fw-bold small">Manager</label><select name="user_id"
                                                                                              id="p_user_id"
                                                                                              class="form-select"
                                                                                              required>
                                <option value="">Select Manager</option>@foreach($users as $u)
                                    <option value="{{$u->id}}">{{$u->name}}</option>
                                @endforeach</select></div>
                        <div class="mb-3"><label class="fw-bold small">Status</label><select name="status_id"
                                                                                             id="p_status_id"
                                                                                             class="form-select">@foreach($statuses as $s)
                                    <option value="{{$s->id}}">{{$s->label}}</option>
                                @endforeach</select></div>
